...
### Scatterplot: Musical Sophistication and Valence Estimation Accuracy {data-height=800}
...
### Violin Plots: Valence Estimation Deviation by Genre

```{r violin plots, fig.width=11}

genre_truVal_means <- data_long %>%
  group_by(genre) %>%
  summarise(signValDev=mean(truVal))
...
### Bar Chart: Average Valence Estimate Deviation
...
genres_data %>%
  ggplot(aes(x = genre, y = valence_total_dev/count )) +
  geom_col() +
